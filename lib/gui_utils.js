...
const fixAllOfDeps = (schema) => {
    if (!schema.allOf) {
        return;
    }

    schema.allOf.forEach(subSchema => fixAllOfDeps(subSchema));

    const deps = deepmerge.all(
        schema.allOf.map(subSchema => subSchema.dependencies || {})
    );
    schema.allOf[0].dependencies = deps;
};
