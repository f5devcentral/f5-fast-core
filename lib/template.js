...
class Template {
    ...
    _findTemplateText(def) {
        if (def.template) {
            return def.template;
        }

        if (def.$ref && def.$ref.startsWith('#/definitions/')) {
            const refDef = this._defFromRef(def.$ref);
            if (refDef.template) {
                return refDef.template;
            }
        }
    ...
    _handleXOf(schema, schemaDef, typeSchemas) {
        ['oneOf', 'anyOf', 'allOf'].forEach((xOf) => {
            if (schemaDef[xOf]) {
                schema[xOf] = schemaDef[xOf].map((xOfDef) => {
                    const newDef = Object.assign({}, xOfDef);
                    const templateText = this._findTemplateText(newDef);
                    if (templateText) {
                        const xOfParsed = this._handleParsed(
                            Mustache.parse(templateText),
                            typeSchemas
                        );
                        Object.assign(newDef, xOfParsed);
    ...
    _handleParsed(parsed, typeSchemas) {
        ...
        const schema = parsed.reduce((acc, curr) => {
            ...
            case 'name': {
                ...
                if (this.definitions[defName]) {
                    ...
                    this._handleXOf(acc.properties[defName], this.definitions[defName]);
                }
        ...
        const hasXOf = schema.oneOf || schema.anyOf || schema.allOf;
        ...
        if (!hasXOf && Object.keys(schema.properties).length < 1) {
            return {
                type: 'string',
                default: primitives.string
            };
    ...
    static loadYaml(yamltext, schemaProvider) {
        ...
        return Promise.all([
            ...
            $RefParser.bundle(yamldata, refParserOpts)
                .catch(e => Promise.reject(new Error(
    ...
    _getPartials() {
        return Object.keys(this.definitions).reduce((acc, curr) => {
            ...
            const templateText = this._findTemplateText(def);
            if (templateText) {
