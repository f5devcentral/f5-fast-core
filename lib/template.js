...
class Template {
    ...
    render(parameters, options) {
        ...
        const combParams = this.getCombinedParameters(parameters);
        const xfparams = (options.skipTransforms) ? combParams : this.transformParameters(combParams);
        ...
        this._allOf.forEach((tmpl) => {
            templateTexts.push(tmpl.render(xfparams, subTmplOpts));
        });
