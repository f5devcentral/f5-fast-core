...
class Template {
        ...
        const schema = parsed.reduce((acc, curr) => {
            ...
            case 'name': {
                ...
                if (this.definitions[defName]) {
                    Object.assign(propDef, this.definitions[defName]);
                }
                ...
                if (this.definitions[defName]) {
                    Object.assign(this.definitions[defName], propDef);
                }
                if (this._typeDefinitions[defName]) {
                    Object.assign(this._typeDefinitions[defName], propDef);
                }
                break;
            }
            case '>': {
    ...
    static loadMst(msttext, schemaProvider) {
        ...
        return tmpl._loadTypeSchemas(schemaProvider)
            .then((typeSchemas) => {
                ...
                tmpl._parametersSchemaFromTemplate(typeSchemas, true);
            })
     ...
     * @returns {Promise} Promise resolves to `Template`
     */
    static loadYaml(yamltext, schemaProvider, filePath, rootDir, skipValidation, skipCleanDefs) {
        ...
        const loadSubTemplate = subTemplate => Template.loadYaml(
            JSON.stringify(subTemplate),
            schemaProvider,
            undefined,
            undefined,
            true,
        ...
        return Promise.resolve()
            ...
            .then((typeSchemas) => {
                ...
                if (!skipCleanDefs) {
                    ['oneOf', 'allOf', 'anyOf'].forEach((xOf) => {
                        tmpl[`_${xOf}`].forEach((subTmpl) => {
                            subTmpl._cleanUnusedDefinitions();
                        });
                    });
                    tmpl._cleanUnusedDefinitions();
                }
            })
            .then(() => {
                if (skipValidation) {
