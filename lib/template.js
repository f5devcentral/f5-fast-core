        ...
        const schema = parsed.reduce((acc, curr) => {
            ...
            case 'name': {
                ...
                const isRequired = (
                    ...
                    && !(this.definitions[defName] && this.definitions[defName].default)
                );
                if (isRequired) {
            ...
            case '#': {
                ...
                if (defType === 'array') {
                    ...
                    const hasDefaults = Object.entries(newDef.items.properties || [])
                        .reduce((a, [propName, propDef]) => {
                    ...
                    if (newDef.items.required) {
                        newDef.items.required = newDef.items.required
                            .filter(x => !hasDefaults.includes(x));
                    }
