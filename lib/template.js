...
class Template {
    ...
    static loadYaml(yamltext, schemaProvider, filePath) {
        ...
        return Promise.all([
            ...
            Promise.resolve()
                .then(() => {
                    if (filePath) {
                        return $RefParser.bundle(filePath, yamldata, refParserOpts);
                    }
                    return $RefParser.bundle(yamldata, refParserOpts);
                })
                .catch(e => Promise.reject(new Error(
    ...
    static fromJson(obj) {
        ...
        return Promise.resolve()
            .then(() => Promise.all(obj._oneOf.map(x => Template.loadYaml(JSON.stringify(x)))))
            .then((tmplList) => {
                tmpl._oneOf = tmplList;
            })
            .then(() => Promise.all(obj._allOf.map(x => Template.loadYaml(JSON.stringify(x)))))
            .then((tmplList) => {
                tmpl._allOf = tmplList;
            })
            .then(() => Promise.all(obj._anyOf.map(x => Template.loadYaml(JSON.stringify(x)))))
            .then((tmplList) => {
                tmpl._anyOf = tmplList;
            })
            .then(() => tmpl._createParametersValidator())
    ...
    render(parameters) {
        ...
        const mergeStrategy = PlainTextMergeStrategy;
        let result = Mustache.render(templateText, xfparams, partials);
