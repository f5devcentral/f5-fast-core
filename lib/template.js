...
class Template {
                ...
                schema[xOf] = schemaDef[xOf].map((xOfDef) => {
                    ...
                    if (newDef.template) {
                        const xOfParsed = this._handleParsed(
                            Mustache.parse(newDef.template),
                            typeSchemas
                        );
    ...
    _handleParsed(parsed, typeSchemas) {
        ...
        const schema = parsed.reduce((acc, curr) => {
            ...
            case '>': {
                const partialDef = this.definitions[mstName];
                const partial = this._handleParsed(Mustache.parse(partialDef.template), typeSchemas);
                this._handleXOf(partial, partialDef, typeSchemas);
